...
//		Stream<List<OrderItem>> orderItemListStream = orderStream.map(order -> order.getOrderItems());
//		Stream<OrderItem> orderItemStream = orderItemListStream.flatMap(List::stream);
//		orderItemStream.forEach(System.out::println);
		orders.stream().map(order -> order.getOrderItems())
			.flatMap(List::stream)
