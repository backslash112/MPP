...
//			.filter(str -> Collections.frequency(list1, str) + Collections.frequency(list2, str) == 1)
//			.collect(Collectors.toList());
		
		// Method 2, much easier
		 return Stream.concat(
				 list1.stream().filter(x -> list2.indexOf(x) == -1), 
				 list2.stream().filter(x -> list1.indexOf(x) == -1))
				 .collect(Collectors.toList());
	}
